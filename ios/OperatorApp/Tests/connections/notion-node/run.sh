#!/bin/sh
set -eu
test_dir=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd); ios_dir=$(CDPATH= cd -- "$test_dir/../../../.." && pwd); scratch=$(mktemp -d /private/tmp/operator-notion-node-proof.XXXXXX); trap 'rm -rf "$scratch"' EXIT HUP INT TERM
mkdir -p "$scratch/Sources/OperatorApp" "$scratch/Tests/NotionNodeProofTests" "$scratch/module-cache"
...
cp "$ios_dir/OperatorApp/Sources/connections/notion/setup/NativeNotionSetupCoordinator.swift" "$scratch/Sources/OperatorApp/"
...
cp "$ios_dir/OperatorApp/Sources/connections/notion/node/ForegroundNotionService.swift" "$scratch/Sources/OperatorApp/"
cp "$ios_dir/OperatorApp/Sources/capabilities/node/ForegroundNodeCommandRouter.swift" "$scratch/Sources/OperatorApp/"
cp "$test_dir/NotionNodeTests.swift" "$scratch/Tests/NotionNodeProofTests/"
