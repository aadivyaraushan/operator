...
scratch=$(mktemp -d /private/tmp/operator-account-setup-proof.XXXXXX)
trap 'rm -rf "$scratch"' EXIT HUP INT TERM
mkdir -p "$scratch/Sources/OperatorApp" "$scratch/Tests/NativeAccountSetupProofTests" "$scratch/module-cache"
...
cp "$test_dir/NativeAccountSetupCoordinatorTests.swift" "$scratch/Tests/NativeAccountSetupProofTests/"
